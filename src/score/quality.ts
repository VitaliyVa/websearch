...
export function scoreSite(a: SiteAudit | null, psi: PsiResult | null, hasWebsite: boolean): QualityScore {
  ...
  /*
   * Заглушка бот-захисту, що прикидається успішною відповіддю.
   *
   * Не всі захисти віддають 403. Moskalenko Group повернув HTTP 202 із 168
   * байтами HTML без <title> — це JS-челендж Cloudflare, а не сайт. Аудит
   * зарахував його як справжню сторінку і виставив 1/10 з причинами
   * «не адаптивний» та «нема опису для Google», хоча жодної з них ми не бачили.
   *
   * Ціна помилки — не цифра в таблиці, а фраза продажника «у вас
   * односторінковий сайт без форм», сказана власнику нормального сайту.
   *
   * Поріг свідомо низький: справжня сторінка малого бізнесу не буває меншою
   * за 2 КБ розмітки, тож хибних спрацювань тут практично не буває.
   */
  if (a && !a.fetchError && a.htmlBytes > 0 && a.htmlBytes < 2000 && !a.title) {
    return {
      score10: 5,
      raw: 50,
      reasons: [
        `сайт віддав ${a.htmlBytes} байт без заголовка (HTTP ${a.httpStatus}) — схоже на заглушку bot-protection, потрібна ручна перевірка`,
      ],
      status: 'blocked',
      datedMarkers: [],
    };
